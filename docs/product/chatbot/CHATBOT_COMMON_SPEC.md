...
- `ChatFloatingButton`: 실제 플로팅 챗봇 UI, SSE 진행 상태/도구 결과/최종 답변 처리
...
4. `ChatbotService`가 Spring AI 플래너를 시도한다.
5. OpenAI API key 또는 ChatClient가 없거나 플래너가 빈 계획을 반환하면 fallback planner는 `GENERAL` intent와 빈 `actions`만 반환한다.
6. 서버는 planner 결과에 로컬 intent/action을 강제로 추가하지 않는다.
7. `ChatActionValidator`는 planner가 제안한 액션만 허용 목록 기준으로 정규화하고, 중복 `NAVIGATE MAP`을 제거하며, `MAP_SET_SEARCH_QUERY`는 `MAP+FLOWER+keyword`일 때만 유지한다.
8. `FLOWER` intent에서는 요청 문맥에 따라 `flower.searchFlowerSpots`, `flower.lookupDescriptionSource`, `flower.lookupGrowTipsSource` 중 필요한 하나만 선택한다.
9. `COMMUNITY` intent에서는 커뮤니티 검색 도구를 실행한다.
10. 서버는 `STATUS`, `ACTION`, `TOOL_RESULT`, `FINAL_ANSWER`, `DONE` 이벤트를 순차 전송한다.
